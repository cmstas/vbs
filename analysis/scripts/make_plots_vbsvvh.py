...
def opt_plots(plotter, plots_dir, channel):
...
def val_plots(plotter, plots_dir, channel):
...
def extra_plots(plotter, plots_dir, channel):
...
if __name__ == "__main__":
    ...
    # Define preselection
    plotter.df["objsel"] = plotter.df.eval(f"is_{channel}")
    if channel == "allmerged":
    ...
    if plot_all or args.opt:
        opt_plots(plotter, f"{base_dir}/opt_{channel}", channel)
    if plot_all or args.val:
        val_plots(plotter, f"{base_dir}/val_{channel}", channel)
    if plot_all or args.extra:
        extra_plots(plotter, f"{base_dir}/extra_{channel}", channel)
